...
def compute_word_span_metrics(
    ...
    df[["aspan_ind_start", "aspan_ind_end"]] = df.aspan_inds.str.extract(
        pattern, expand=True
    ).fillna(0).astype(int)  # TODO only the first span is extracted
...
def load_data(
    ...
    if data_path.is_dir():
        try:
            dataframes = [pd.read_csv(file, encoding='utf-16', **kwargs) for file in data_path.glob('*.csv')]
        except UnicodeError:
            dataframes = [pd.read_csv(file, low_memory=False, **kwargs) for file in data_path.glob('*.csv')]
